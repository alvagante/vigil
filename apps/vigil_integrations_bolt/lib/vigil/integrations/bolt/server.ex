defmodule Vigil.Integrations.Bolt.Server do
  @moduledoc """
  Lifecycle process for a single Bolt integration instance.

  Registers the instance in `Vigil.Plugin.Registry` for dispatch and config
  access, holds the current config in process state for hot reloads, and
  tracks the concurrent-execution slot count for BOLT-303.
  """
  ...
  def init({integration_id, config}) do
    {:ok, _} =
      Registry.register(@registry, {:integration, integration_id}, Vigil.Integrations.Bolt)
