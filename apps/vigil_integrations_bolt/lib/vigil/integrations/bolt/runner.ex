defmodule Vigil.Integrations.Bolt.Runner do
      ...
      case Server.acquire_slot(integration_id, max) do
        :ok ->
          pid =
            spawn(fn ->
              try do
                run_targets(integration_id, config, command, targets, stream_pid)
              after
  ...
  defp run_targets(_integration_id, config, command, targets, stream_pid) do
