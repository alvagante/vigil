...
| **Authorization** | Full RBAC (roles, permissions, assignments); per-integration scoping; per-target/per-command granular permissions; default built-in roles; **direct (literal) OIDC group-to-role mapping with additive multi-group resolution** (a user in N matched groups receives the union of all corresponding roles) |
...
|------|--------------------------|---------------------|
| **External authentication (enterprise)** | SAML 2.0 (multi-IdP); LDAP/Active Directory bind + search; **multi-IdP OIDC** (multiple concurrent OIDC providers, e.g. Okta for staff plus an OIDC for contractors); JIT user provisioning beyond OIDC baseline; **group-to-role mapping with wildcard patterns** (e.g. `ops-*` → Operator role); **IdP group re-evaluation on every login** (CE evaluates groups only at JIT provisioning; EE re-evaluates on each login so IdP group changes take effect immediately); cross-IdP coexistence rules | Enterprises demand SAML and AD. Multi-IdP, wildcard group patterns, and continuous group re-evaluation are organisational-governance features. CE covers the single-IdP OIDC case with additive literal group-to-role mapping (DM-301 is satisfied in CE); EE layers on the enterprise identity surface. |
...
| **Approval workflows** | Configuration-driven approval gate: administrators define rules that match on action type (execution, provisioning, code deployment), target scope (specific nodes, groups, or integration), and submitter role. Matching actions are queued pending approval from one or more members of a designated approver role; multi-approver quorum is configurable. Approval windows expire after a configurable duration. All approval decisions (grant, deny, expire) are recorded in the audit trail. Submitters cannot approve their own actions. The gate is policy-enforced — it is not a user-selected option. CE's pass-through `ApprovalGate` extension point is replaced by the EE approval engine at runtime. | Change-management and compliance requirement in regulated environments. Small teams use trust; enterprises use process. |
...
- **OIDC → CE, SAML + LDAP → EE.** Generic single-IdP OIDC is cheap to implement, covers the "our team uses Google / GitHub / Keycloak SSO" case that every small self-hosted team runs into, and denying it creates a hostile CE experience. SAML and LDAP/AD, by contrast, are enterprise-scale identity integrations with heavier operational surface (IdP metadata management, multi-realm bind, certificate rotation) that small teams do not need. CE OIDC supports additive multi-group resolution (a user in N groups gets the union of all matched roles, satisfying DM-301 universally). EE adds wildcard group-name patterns, continuous group re-evaluation on every login, multi-IdP coexistence, and cross-IdP rules — these are organisational-governance features that stay EE alongside SAML/LDAP.
...
| **FS 14** — OIDC Authentication (CE) | Single-IdP OIDC; JIT provisioning; direct (literal) group-to-role mapping with additive multi-group resolution (union of all matched roles); group evaluation at JIT provisioning time; coexistence with local auth | CE |
...
| **FS EE-3** — Approval Workflows | Configuration-driven approval rules matched on action type, target scope, and submitter role; queued pending approval from designated approver role(s); configurable quorum; expiring windows; full approval audit trail; submitter cannot self-approve | EE |
